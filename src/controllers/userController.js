...
let userIdFromLocal
// for creating a cart of user whose localstorage data will be store in cartu
const getUserId = () => {
...
const registerUser = async (req, res) => {
  try {
    ...
    // save user after generate token
    user.save()
...
const loginUser = async (req, res) => {
  try {
    ...
    // checking if user exist with the email
    const user = await userModel.findOne({ email })
    if (!user) {
      return res
        .status(404)
        .send({ status: false, msg: 'User not found with this email' })
    }
    ...
    if (oldTokens.length) {
      // removing the expired token from db
...
//   if user not enter any email
    if (!email) {
      return res
        .status(400)
        .send({ status: false, msg: 'Enter your registerd email' })
    }
    if (email && email.length < 10) {
      return res.status(400).send({ status: false, msg: 'Enter valid email' })
    }

    // checking user is registered with this email or not
...
const updatePassword = async (req, res) => {
  try {
    //  taking emailToken from params
    let emailToken = req.params.emailToken

    // taking password and confirmPassword from body
    let { password, confirmPassword } = req.body

    //  if both field are not provided
    ...
    // checking both password not match
    if (password !== confirmPassword) {
      return res
        .status(400)
        .send({
    ...
    if (!userEmailToken) {
      return res
        .status(401)
        .send({ status: false, msg: 'link expired,create a new link' })
    }
    ...
    if (userEmailToken.emailTokenExp < new Date()) {
      return res
        ...
        .send({
          status: false,
          msg: 'link is expired ,Please create a new one'
        })
    }
    ...
    await userModel.findOneAndUpdate(
      { _id: userEmailToken._id },
      { $set: { password: password, emailToken: '', emailTokenExp: 0 } },
      { new: true }
    )
    return res
      .status(200)
      .send({ status: true, msg: 'Your password updated Successfully' })
  } catch (error) {
    return res.status(500).send({ error: error.message })
  }
}
...
const logout = async (req, res) => {
  try {
    ...
    //filtering the tokens with the token in headers
...
const generateNewToken = async (req, res) => {
  try {
    ...
    if (!verifiedToken) {
      return res
        .status(401)
        .send({ status: false, msg: 'invalid token please login again' })
    }
    ...
    user.tokens = [
      { newToken, validUpto: new Date(Date.now() + 5 * 60 * 1000) }
    ]
    await user.save()
    return res
      .status(200)
